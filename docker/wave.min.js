var waveconfig = {
    ...
    reportPageMessaging = {
        fn: {},
        init: function() {
            reportPageMessaging.fn.setupEventListeners(), reportPageMessaging.fn.setupAndDispatchEvent("getExtensionUrl", ""), reportPageMessaging.fn.checkThatConfigUrlEventHasFired()
        },
        extensionUrlTimeout: {
            limit: 1e4,
            current: 0
...
reportPageMessaging.fn.setupAndDispatchEvent = function(e, t) {
    ...
    if (reportPageMessaging.extensionUrlTimeout.current >= reportPageMessaging.extensionUrlTimeout.limit) return console.log("extension timed out: not able to get extensionUrl."), void(reportPageMessaging.extensionUrlTimeout.current = 0);
    "" == waveconfig.extensionUrl && (waveconfig.debug && console.log("extensionUrl not yet set."), setTimeout(reportPageMessaging.fn.checkThatConfigUrlEventHasFired, 25), reportPageMessaging.extensionUrlTimeout.current += 25), waveconfig.debug && console.log("extensionUrl from messaging: " + waveconfig.extensionUrl)
    ...
    document.addEventListener("setExtensionUrl", (function(e) {
        ...
        wave.tabsengine.toggleStyles(reportPageMessaging.fn.getMessageData(e.detail))
        ...
        waveconfig.debug && console.log("Background confirms results recieved by sidebar. Start buildOutline"), wave.tabsengine.buildOutline()
    })), document.addEventListener("contrastIconDetails", (function(e) {
        if (wave.tabsengine.currentContrastIcon) {
            var t = reportPageMessaging.fn.getMessageData(e.detail).foregroundColor,
                a = reportPageMessaging.fn.getMessageData(e.detail).backgroundColor;
            wave.tabsengine.currentContrastIcon.target.style.color = t, wave.tabsengine.currentContrastIcon.target.style.backgroundColor = a
        }
    })), document.addEventListener("toggleGroup", (function(e) {
        switch (reportPageMessaging.fn.getMessageData(e.detail).action) {
            case "show":
                wave.report.things.showGroup(reportPageMessaging.fn.getMessageData(e.detail).group);
                break;
            case "hide":
                wave.report.things.hideGroup(reportPageMessaging.fn.getMessageData(e.detail).group)
        }
    })), document.addEventListener("toggleType", (function(e) {
        switch (reportPageMessaging.fn.getMessageData(e.detail).action) {
            case "show":
                wave.report.things.showType(reportPageMessaging.fn.getMessageData(e.detail).type);
                break;
            case "hide":
                wave.report.things.hideType(reportPageMessaging.fn.getMessageData(e.detail).type)
        }
    })), document.addEventListener("hoverHighlight", (function(e) {
        iconObject = wave.report.things.iconsByType(reportPageMessaging.fn.getMessageData(e.detail).iconName)[reportPageMessaging.fn.getMessageData(e.detail).index], wave.code.hoverHighlight(iconObject.target, reportPageMessaging.fn.getMessageData(e.detail).highlight)
    })), document.addEventListener("iconClickAction", (function(e) {
        var t = wave.report.things.iconsByType(reportPageMessaging.fn.getMessageData(e.detail).iconType)[reportPageMessaging.fn.getMessageData(e.detail).index];
        ...
        wave.tabsengine.toggleSaturation(reportPageMessaging.fn.getMessageData(e.detail).action)
    })), document.addEventListener("showNavOrder", (function(e) {
        wave.tabsengine.showNavOrder(reportPageMessaging.fn.getMessageData(e.detail).action, reportPageMessaging.fn.getMessageData(e.detail).ShowIcons)
    })), document.addEventListener("showWaveIcons", (function(e) {
        wave.tabsengine.showWaveIcons(reportPageMessaging.fn.getMessageData(e.detail).action)
    })), document.addEventListener("scrollToNavIcon", (function(e) {
        wave.tabsengine.scrollToNavIcon(reportPageMessaging.fn.getMessageData(e.detail).action)
    }))
}, reportPageMessaging.init(), (wave = {
    version: "3.2.4.4",
    allTags: null,
    code: null,
    engine: {},
    ...
    try {
        for (var e = 0; e < wave.allTags.length; e++) wave.allTags[e].removeAttribute("data-waveid", e + 1)
    } catch (e) {
        console.log("Failed to remove waveids", e)
    }
}, wave.fn.reset = function() {
    "extension" != waveconfig.platform && document.getElementById("report") && document.getElementById("report").replaceChildren(), document.getElementById("wave5_iconbox") && (document.getElementById("wave5_iconbox").style.display = "none"), document.getElementById("wave5topbar") && (document.getElementById("wave5topbar").replaceChildren(), document.getElementById("wave5topbar").onclick = function(e) {
        wave.iconbox.hide()
    }), "extension" != waveconfig.platform || waveconfig.isSidebar ? (wave.code = new wave.Code, wave.engine.fn.resetEngine(), wave.tabsengine.codePanelHasRun = !1) : (wave.code = new wave.Code, wave.engine.fn.resetEngine()), wave.fn.resizeReportWindow(), wave.allTags = null, wave.report = {}, wave.xpath.mappedElementsByXPath = {}
}, wave.fn.run = function() {

    return wave.fn.setupTags(), wave.results = {}, wave.results.statistics = {}, wave.engine.run().then((function() {
        wave.results.categories = wave.engine.results, wave.results.statistics = wave.engine.statistics;
        wave.engine.ruleTimes;
        // contrast alert feature structure aria
        return wave.results.statistics.pagetitle = wave.page.title, wave.results.statistics.totalelements = wave.allTags.length, wave.results.success = !0, wave.engine.fn.structureOutput(), wave.results.success ? (wave.report.iconlist = wave.results.categories, wave.report.title = wave.results.statistics.pagetitle, wave.engine.icons.setStats(wave.results.statistics)) : wave.tabs.showError("Could not get the report results. Please try your request again."), wave.fn.showThings(), "extension" == waveconfig.platform ? reportPageMessaging.fn.setupAndDispatchEvent("waveResults", wave.report.iconlist) : wave.tabs.updatePanels(wave.report.iconlist), wave.fn.checkacsb(), wave.tabsengine.codePanelOpen ? wave.fn.showCode() : document.getElementById("wave5spinner").display = "block", document.querySelector("#wave5codetoggle a").onclick = function(e) {
            e.preventDefault(), wave.fn.loadCodePanel(e)
        }, wave.results
    }))
}, wave.fn.setupTags = function() {
    try {
        wave.allTags = wave.page.getElementsByTagName("*"), wave.fn.assignWaveIds(wave.allTags), wave.xpath.calculateXPath(wave.page.documentElement), wave.engine.fn.setupResultStack()
    } catch (e) {
        throw wave.tabs.showError("There is a scripting conflict with content on this page. We recommend using the WAVE <a href='https://wave.webaim.org/extension/'>Chrome or Firefox Extension</a> to evaluate this page."), new Error("Scripting conflict with content on the page: " + e)
    }
}, wave.fn.checkacsb = function() {
    "undefined" == typeof acsbJS && "undefined" == typeof AJS || ("extension" != waveconfig.platform || waveconfig.isSidebar ? wave.tabs.showacsbalert("accessiBe") : reportPageMessaging.fn.setupAndDispatchEvent("showacsbalert", "accessiBe"))
}, wave.fn.resizeReportWindow = function() {
    if ("extension" != waveconfig.platform) {
        var e = window.innerHeight - document.getElementById("wave5topbar").offsetHeight - document.getElementById("wave5bottombar").offsetHeight;
        document.getElementById("report").style.height = e + "px"
    }
}, wave.code = null, wave.rules = {}, wave.tabs = {}, wave.url = null, wave.fn.initialize = function() {
    ...
    wave.fn.reset();
    var e = {};
    return waveconfig.isSidebar || (wave.page = document, e = wave.fn.run()), e
    ...
    waveconfig.debug && console.log("Start reportReadyEvent. Sidebar: " + waveconfig.isSidebar), wave.fn.removeWaveIds(wave.allTags), waveconfig.isSidebar && (wave.tabs.removeLoadingAnimation(), wave.tabs.doResize()), wave.iconbox = new wave.manager.IconBox, wave.fn.insertIconBoxForChromeExtension(), wave.report.things.updateTopbar(), waveconfig.debug && console.log("End reportReadyEvent")
    ...
    if (!waveconfig.isSidebar) {
        ...
        r.id = "wave5_iconbox_arrow", t.appendChild(a), e.appendChild(t), i.appendChild(s), i.appendChild(o), i.appendChild(r), e.appendChild(i), e.style.display = "none", e.onclick = function(e) {
            e.stopPropagation()
        }, document.body.appendChild(e)
    }
}, wave.utilities = {}, "undefined" == typeof trustedTypes && (trustedTypes = {
    createPolicy: (e, t) => t
}), wave.utilities.escapeHTMLPolicy = trustedTypes.createPolicy("forceInner", {
    createHTML: e => e
}), String.prototype.repeat = function(e) {
    return new Array(e + 1).join(this)
}, wave.manager = {}, wave.manager.style = {}, wave.manager.selectedIcon = null, wave.manager.style.icon = {
    border: "none",
    ...
    for (var n in this.contextPage = a, this.pageWideContextPage = document, this.allTagsCached = {}, this.iconProperties = {}, this.iconList = [], this.iconBaseXpaths = {}, this._iconGroups = [], this._iconsByGroup = {}, this._typesByGroup = {}, this._iconsByType = {}, this.allTagsCached = wave.xpath.mappedElementsByXPath, this.allTagsCached["#"] = document.getElementById("wave5topbar"), e)
        if (wave.engine.icons.stats[n] = e[n].count, e[n].count > 0)
            for (var i in e[n].items)
                for (var s = e[n].items[i], o = 0; o < s.xpaths.length; o++) {
                    var r = s.xpaths[o];
                    if (void 0 !== (s.contrastdata && s.contrastdata[o])) var l = new wave.manager.Icon(s.id, t[s.id], r, this.allTagsCached[r], s.text[o], s.contrastdata[o]);
                    else l = new wave.manager.Icon(s.id, t[s.id], r, this.allTagsCached[r], s.text[o], "");
                    this.attach(l), this.iconList.push(l), void 0 === this.iconBaseXpaths[r] && (this.iconBaseXpaths[r] = []), this.iconBaseXpaths[r].push(l), this._iconsByGroup[n] || (this._iconsByGroup[n] = []), this._iconsByGroup[n].push(l), this._typesByGroup[n] || (this._typesByGroup[n] = []), -1 == this._typesByGroup[n].indexOf(s.id) && (this._typesByGroup[n].push(s.id), this._iconsByType[s.id] = []), this._iconsByType[s.id].push(l)
                }
    ...
    switch (e.position) {
        ...
        case "after":
            ...
            break;
        case "first":
            if (e.target.nodeName && -1 !== e.target.nodeName.search(/(INPUT|IMG|SVG|TEXTAREA|SELECT)$/i)) {
                t = e.target.insertAdjacentElement("beforebegin", e.iconElement);
                break
            }
    ...
    return e.hasText ? e.iconElement.insertAdjacentElement("afterend", e.textElement) : t
    ...
    for (var t in this._iconsByGroup) document.getElementById("#toggle_group_" + t).checked = !0, document.getElementById("#toggle_group_" + t).click();
    for (var a in this._iconsByType) document.getElementById("#toggle_type_" + t).checked = !0, document.getElementById("#toggle_type_" + t).click();
    this.updateTopbar()
    ...
    console.log(JSON.stringify(wave.results));
    var e = !1,
        t = document.getElementById("wave5topbar");
    if (t.querySelectorAll("img").forEach((function(t, a) {
            0 == e && "none" != t.style.display && "hidden" != t.style.visibility && "hidden" != t.getAttribute("hidden") && (e = !0)
        })), e) {
        if (t.style.display = "block", null == document.getElementById("pagewide_txt")) {
            pagewide_txt = document.createElement("div"), pagewide_txt.id = "pagewide_txt";
            var a = document.createTextNode("The following apply to the entire page: ");
            pagewide_txt.appendChild(a), t.prepend(pagewide_txt)
        }
    } else t.style.display = "none";
    wave.fn.resizeReportWindow()
}, wave.manager.Icon = function(e, t, a, n, i, s) {
    this.type = e, this.data = t, this.itemXPath = a, this.target = n, this.hasText = !1 !== i && 0 != i.length, this.iconText = i, this.iconContrastdata = s, this.position = this.data.position, this.hidden = "#" !== a && (0 == n.offsetWidth || 0 == n.offsetHeight || n.offsetTop < 0 || n.offsetLeft < 0), this.textElement = {}
}, wave.manager.Icon.prototype.create = function(e) {
    var t = document.createElement("img");
    if (this.iconElement = t, t.setAttribute("src", wave.conf.paths.site + "/img/icons/" + this.type + ".svg"), t.setAttribute("class", "wave5icon"), t.setAttribute("tabindex", "0"), t.setAttribute("role", "button"), this.iconContrastdata && (this.iconElement.contrastData = this.iconContrastdata), Object.assign(t.style, wave.manager.style.icon), wave.engine.icons.docs[this.type]) {
        var a = wave.engine.icons.docs[this.type].category.toUpperCase() + ": " + wave.engine.icons.docs[this.type].title;
    ...
    if (this.hasText) {
        ...
        wave5text.appendChild(n), wave5text.className = "wave5text", Object.assign(wave5text.style, wave.manager.style.text), this.textElement = wave5text
    }
}, wave.manager.Icon.prototype.hide = function() {
    try {
        this.iconElement.style.display = "none", this.hasText && (this.textElement.style.display = "none")
    } catch (e) {
        console.log("Could not hide element: " + e)
    }
}, wave.manager.Icon.prototype.show = function() {
    try {
        this.iconElement.style.display = "", this.hasText && (this.textElement.style.display = "")
    } catch (e) {
        console.log("Could not hide element: " + e)
    }
}, wave.manager.Icon.prototype.scrollToIcon = function() {
    if ("extension" == waveconfig.platform) window.scrollTo(0, this.iconElement.getBoundingClientRect().top + document.documentElement.scrollTop - 110);
    else {
        document.getElementById("report").contentWindow.document.documentElement.scrollTo(0, this.iconElement.getBoundingClientRect().top + document.getElementById("report").contentWindow.document.documentElement.scrollTop - 110);
        var e = document.createElement("style");
        e.textContent = "@keyframes blink { 50% { opacity:0 ; } }", document.getElementById("report").contentWindow.document.body.appendChild(e)
    }
    this.iconElement.style.animationPlayState = "running", this.iconElement.style.animation = "blink .25s", this.iconElement.style.animationIterationCount = "3", this.iconElement.onanimationend = function(t) {
        t.target.style.animationPlayState = "paused", t.target.style.animation = "", "extension" != waveconfig.platform && e.remove()
    }
}, wave.manager.IconBox = function() {
    var e = this;
    this.hide(), wave.page.onscroll = function(t) {
        e.hide()
    ...
    for (var t = wave.report.things.icons(), a = 0; a < t.length; a++) t[a].iconElement.addEventListener("focus", function(e) {
        return function() {
            wave.code.hoverHighlight(e.target, !0)
        }
    }(t[a])), t[a].iconElement.addEventListener("mouseover", function(e) {
        return function() {
            wave.code.hoverHighlight(e.target, !0)
        }
    }(t[a])), t[a].iconElement.addEventListener("blur", function(e) {
        return function() {
            wave.code.hoverHighlight(e.target, !1)
        }
    }(t[a])), t[a].iconElement.addEventListener("mouseout", function(e) {
        return function() {
            wave.code.hoverHighlight(e.target, !1)
        }
    }(t[a])), t[a].iconElement.addEventListener("click", function(e, t) {
        return function(t) {
            iconClick(e), t.preventDefault(), t.stopPropagation()
        }
    }(t[a])), t[a].iconElement.addEventListener("keyup", function(e, t) {
        return function(t) {
            13 != t.keyCode && 32 != t.keyCode || iconClick(e)
        }
    }(t[a])), iconClick = function(e) {
        if (1 == wave.tabsengine.codePanelOpen && "#" !== e.itemXPath && wave.code.setSelected(e), "contrast" == e.data.name || "contrast" == e.type)
            if (wave.tabsengine.currentContrastIcon = e, "extension" == waveconfig.platform) {
                var t = e.iconContrastdata,
                    a = (t[0], t[1]),
                    n = t[2],
                    i = t[3];
                reportPageMessaging.fn.setupAndDispatchEvent("setSidebarContrastDetails", {
                    fgcolor: a,
                    bgcolor: n,
                    largetext: i
                })
            } else showContrasts(e.iconContrastdata);
        "tab-reference" == wave.tabs.current && ("extension" == waveconfig.platform ? reportPageMessaging.fn.setupAndDispatchEvent("moreInfo", e.data.name) : wave.tabs.moreInfo(e.type)), null != e.iconContrastdata[0] ? document.getElementById("wave5_iconbox_title").innerHTML = wave.utilities.escapeHTMLPolicy.createHTML(wave.engine.icons.docs[e.type].title + " (" + e.iconContrastdata[0] + ":1)") : document.getElementById("wave5_iconbox_title").innerHTML = wave.utilities.escapeHTMLPolicy.createHTML(wave.engine.icons.docs[e.type].title), document.getElementById("wave5_iconbox_summary").innerHTML = wave.utilities.escapeHTMLPolicy.createHTML(wave.engine.icons.docs[e.type].summary);
        var s = document.getElementById("wave5_iconbox_doclink");
        s.textContent = "";
        var o = document.createElement("a");
        o.href = "#";
        var r, l = document.createTextNode("Reference");
        o.appendChild(l), s.append(o), o.addEventListener("click", (r = e.data.name, function(e) {
            "extension" == waveconfig.platform ? reportPageMessaging.fn.setupAndDispatchEvent("moreInfo", r) : wave.tabs.moreInfo(r), e.preventDefault(), e.stopPropagation()
        }));
        var c = document.createElement("a");
        c.href = "#", c.className = "viewInCodePanelLink";
        l = document.createTextNode("Code");
        c.appendChild(l), s.append(c), c.addEventListener("click", (function(t) {
            wave.tabsengine.codePanelOpen || wave.fn.loadCodePanel(t), wave.code.setSelected(e), t.preventDefault(), t.stopPropagation()
        }));
        var d = document.getElementById("wave5_iconbox");
        d.style.display = "block", d.style.opacity = 1, d.style.overflow = "";
        var u, g = d.getBoundingClientRect().height,
            m = d.getBoundingClientRect().width,
            p = wave.page.body.offsetWidth;
        u = "wave5topbar" == e.target.id || "extension" == waveconfig.platform ? 0 : document.getElementById("sidebar_container").offsetWidth;
        var h = e.iconElement.getBoundingClientRect().left,
            b = e.iconElement.getBoundingClientRect().top,
            v = "extension" == waveconfig.platform || "wave5topbar" == e.target.id ? 0 : document.getElementById("wave5topbar").getBoundingClientRect().height;
        if (b < g + 18) var f = b + v + 52,
            w = "t";
        else f = b + v - g - 12, w = "b";
        if (h + 10 > p - m) var y = h + u - m + 44,
            k = "r";
        else y = h + u, k = "l";
        return d.style.top = f + "px", d.style.left = y + "px", document.getElementById("wave5_iconbox_arrow").removeAttribute("class"), document.getElementById("wave5_iconbox_arrow").classList.add(w + k), !1
    }
}, wave.manager.IconBox.prototype.hide = function() {
    iconbox = document.getElementById("wave5_iconbox"), iconbox && "1" == iconbox.style.opacity && (wave.code, iconbox.style.display = "none")
    ...
    if (0 === a) return -1;
    var n = 0;
    if (arguments.length > 0 && ((n = Number(arguments[1])) != n ? n = 0 : 0 !== n && n !== 1 / 0 && n !== -1 / 0 && (n = (n > 0 || -1) * Math.floor(Math.abs(n)))), n >= a) return -1;
...
var FrmInj = {
    ...
    func: {},
    init: function() {
        FrmInj.func.createTopBar(), FrmInj.func.createBottomBar(), FrmInj.func.createSidebar()
    }
};
if (FrmInj.func.createSidebar = function() {
        var e = getComputedStyle(FrmInj.vars.html).marginLeft;
        e = "auto" === e ? 0 : parseFloat(getComputedStyle(FrmInj.vars.html).marginLeft), FrmInj.vars.html.style.marginLeft = e + parseFloat(FrmInj.vars.sidebarWidth) + "px";
        var t = document.createElement("iframe");
        t.id = "wave_sidebar_container", t.src = waveconfig.extensionUrl + "sidebar.html", t.style.width = FrmInj.vars.sidebarWidth, t.style.height = "100%", t.style.float = "left", t.style.scrolling = "yes", t.style.frameBorder = "0", t.style.allowtransparency = "false", t.style.position = "fixed", t.style.border = "none", t.style.zIndex = "2147483646", t.style.top = "0px", t.style.left = "0px", document.body.insertBefore(t, document.body.firstChild)
    }, FrmInj.func.createTopBar = function() {
        var e = document.createElement("div");
        e.id = "wave5topbar", e.style.width = "100%", document.body.insertBefore(e, document.body.firstChild)
    }, FrmInj.func.createBottomBar = function() {
        var e = getComputedStyle(FrmInj.vars.html).bottom;
        e = "auto" === e ? 0 : parseFloat(getComputedStyle(FrmInj.vars.html).bottom), FrmInj.vars.html.style.bottom = e + parseFloat(FrmInj.vars.bottombarHeight) + "px";
        var t = document.createElement("div");
        t.id = "wave5bottombar", t.setAttribute("role", "region"), t.setAttribute("aria-label", "Code panel"), t.style.float = "left", t.style.scrolling = "no", t.style.frameBorder = "0", t.style.allowtransparency = "false", t.style.position = "fixed", t.style.border = "none", t.style.zIndex = "2147483647", t.style.bottom = "0px", t.style.right = "0px", t.style.backgroundColor = "#FFF";
        var a = document.createElement("div");
        a.id = "wave5spinner", a.style.backgroundColor = "#FFF", a.style.width = "calc(100% - 380px)", a.style.display = "block";
        var n = document.createTextNode("Loading...");
        a.appendChild(n);
        var i = document.createElement("div");
        i.id = "wave5codetoggle";
        var s = document.createElement("a");
        s.id = "wave5codetogglelink", s.href = "#", s.style.background = "transparent";
        var o = document.createElement("span");
        o.className = "wavehidden";
        n = document.createTextNode("code");
        o.appendChild(n), s.appendChild(o), i.appendChild(s);
        var r = document.createElement("div");
        r.id = "wave5code", t.appendChild(a), t.appendChild(i), t.appendChild(r), t.style.left = FrmInj.vars.sidebarWidth, document.body.appendChild(t)
    }, FrmInj.init(), messaging = {
        vars: {},
        fn: {},
        backgroundPort: null,
        init: function() {}
        ...
        var t = {};
        for (var a in e)
            for (var n in t[a] || (t[a] = {}), e[a]) "0" != n && (t[a][this.XPath[n]] = e[a][n]);
        ...
        for (var t = ""; e && 1 == e.nodeType; e = e.parentNode)
            if (1 == e.parentNode.nodeType) {
                ...
                id = "[" + ([].indexOf.call(a, e) + 1) + "]", "HTML" != e.tagName && (t = "/" + e.tagName.toUpperCase() + id + t)
            } return "/HTML" + t
        ...
        for (var t = {}, a = 0; e.children && a < e.children.length; a++) null == t[e.children[a].nodeName] && (t[e.children[a].nodeName] = 0), t[e.children[a].nodeName]++, wave.xpath.calculateXPathPrivate(e.children[a], "/HTML/" + e.children[a].nodeName + "[" + t[e.children[a].nodeName] + "]")
    }, wave.xpath.calculateXPathPrivate = function(e, t) {
        if (e) {
            this.XPath[e.getAttribute("data-waveid")] = t, this.mappedElementsByXPath[t] = e;
            for (var a = {}, n = 0; e.childNodes && n < e.childNodes.length; n++) 1 == e.childNodes[n].nodeType && (null == a[e.childNodes[n].nodeName] && (a[e.childNodes[n].nodeName] = 0), a[e.childNodes[n].nodeName]++, wave.xpath.calculateXPathPrivate(e.childNodes[n], t + "/" + e.childNodes[n].nodeName + "[" + a[e.childNodes[n].nodeName] + "]"))
        }
    }, wave.Code = function() {
        var e = document.getElementById("wave5code");
        e && e.remove(), this.xpaths = {}, this.htmlRoot = null, this.selectedElement = null, this.selectedXpath = ""
    }, wave.Code.prototype.setHtmlRoot = function(e) {
        this.htmlRoot = e
    }, wave.Code.prototype.setSelected = function(e) {
        wave.manager.selectedIcon = e, this.selectedXpath && null != document.querySelector('[data-reportxpath="' + this.selectedXpath + '"]') && document.querySelector('[data-reportxpath="' + this.selectedXpath + '"]').classList.remove("selectedcode"), null != document.querySelector('[data-reportxpath="' + e.itemXPath + '"]') && document.querySelector('[data-reportxpath="' + e.itemXPath + '"]').classList.add("selectedcode"), this.unhighlight();
        var t = document.querySelector('[data-reportxpath="' + e.itemXPath + '"]');
        t && (document.getElementById("wave5code").scrollTop = t.offsetTop - 50), this.selectedElement = e.target, this.selectedXpath = e.itemXPath, this.highlight()
        ...
        null != this.selectedElement && (this.selectedElement.style.outline = "2px dashed red")
        ...
        return this.selectedElement = e, t ? this.highlight() : this.unhighlight(), !1
    }, wave.Code.prototype.iconCodeImage = function(e, t) {
        var a = e.type,
            n = e.data.title,
            i = document.createElement("img");
        return i.setAttribute("src", wave.conf.paths.site + "/img/icons/" + a + ".svg"), i.setAttribute("alt", n), i.style.cursor = "cursor:pointer", e.hidden[t] && (e.classList.add("wave5_hiddenicon"), i.setAttribute("title", 'This icon or the element it applies to is visually hidden in your page. Disable "Styles" to view it.')), e, i.onmouseover = function(t) {
            wave.code.hoverHighlight(e.target, !0)
        }, i.onmouseout = function(t) {
            wave.code.hoverHighlight(e.target, !1)
        }, i.onclick = function(t) {
            if (wave.code.iconClickAction(e), "extension" == waveconfig.platform && "contrast" === e.data.cat_code) {
            ...
            } else "contrast" === e.data.cat_code && showContrasts(e.iconContrastdata)
        }, i
    }, wave.Code.prototype.iconClickAction = function(e) {
        e.scrollToIcon(), wave.iconbox.hide(), 1 == wave.tabsengine.codePanelOpen && wave.code.setSelected(e), "contrast" === e.data.cat_code && (wave.tabsengine.currentContrastIcon = e)
    }, wave.Code.prototype.displayTreePrivate = function(e, t) {
        "" == t && (t = document.createElement("div")).setAttribute("id", "wave5code");
        if (e && (null == e.classList || !(e.classList.contains("wave5icon") || e.classList.contains("wave5text") || e.classList.contains("waveorder") || "waveordercontainer" == e.getAttribute("id") || "wave5bottombar" == e.getAttribute("id") || "wave5_iconbox" == e.getAttribute("id") || "wave5topbar" == e.getAttribute("id") || "wave_sidebar_container" == e.getAttribute("id") || "wavescript" == e.getAttribute("id") || "wavestyle" == e.getAttribute("id")))) {
            if (1 === e.nodeType || 9 === e.nodeType || 10 === e.nodeType) {
                ...
                if (void 0 !== wave.report.things.iconBaseXpaths[a]) {
                    ...
                    for (var i = 0; i < wave.report.things.iconBaseXpaths[a].length; i++) {
                        var s = wave.report.things.iconBaseXpaths[a][i];
                        void 0 !== s.data && (newCodeIcon = wave.code.iconCodeImage(s, 0), n.prepend(newCodeIcon))
                    }
                }
                var o = document.createElement("span");
                o.classList.add("wave5element"), wave5open = document.createTextNode("<" + htmlEntities(e.nodeName.toLowerCase())), o.append(wave5open);
                for (var r = 0; e.attributes && r < e.attributes.length; r++)
                    if (-1 == e.attributes[r].nodeName.indexOf("data-waveid")) {
                        var l = "wave5style" == e.attributes[r].nodeName ? "style" : e.attributes[r].nodeName;
                        if ("style" == l && "" != e.attributes[r].value || "style" != l) {
                            attributes = document.createElement("span"), attributes.classList.add("wave5attribute"), attributes.textContent = " " + l + '="';
                            var c = document.createElement("span");
                            c.classList.add("wave5value"), c.textContent = htmlEntities(e.attributes[r].value), attributes.append(c), attributes.append('"'), o.append(attributes)
                        }
                    } wave5close = document.createTextNode(">"), o.append(wave5close);
                for (r = 0; r < e.childNodes.length; r++) this.displayTreePrivate(e.childNodes[r], o);
                
                return e.innerHTML && e.innerHTML.length > 0 || -1 == ["area", "base", "br", "col", "command", "embed", "hr", "img", "input", "keygen", "link", "meta", "param", "source", "track", "wbr"].indexOf(e.nodeName.toLowerCase()) ? (wave5text = document.createTextNode("</" + htmlEntities(e.nodeName.toLowerCase()) + ">"), o.append(wave5text), n ? (n.append(o), t.append(n)) : t.append(o)) : n ? (n.append(o), t.append(n)) : t.append(o), t
            }
            var d = e.nodeValue.trim();
            if (8 === e.nodeType)(u = document.createElement("div")).classList.add("wave5comment"), u.textContent = "\x3c!-- " + htmlEntities(d) + "--\x3e", t.append(u);
            else if (d.length > 0) {
                var u;
                (u = document.createElement("span")).classList.add("wave5codetext"), u.textContent = htmlEntities(d);
        ...
        }, wave.engine.contrast = {}, wave.engine.results = {}, wave.engine.iconList = {}, wave.engine.statistics = {
            allitemcount: 0,
            totalelements: 0
        }
        ...
        for (e in i) {
            ...
            n = new Date, wave.engine.ruleTimes[e] = n - a, waveconfig.debug && console.log("RULE: " + e + " (" + wave.engine.ruleTimes[e] + "ms)")
        }
        var o = new Date;
        return waveconfig.debug && console.log("TOTAL RULE TIME: " + (o - t) + "ms"), Promise.all([])
        ...
        return n.length > 0 && -1 == n.search(/^\s+$/i) || i && -1 == i.search(/^\s+$/i) || s && -1 == s.search(/^\s+$/i) ? t = !0 : e.querySelectorAll("img, svg, [role=button][aria-label], [role=link][aria-label], [role=img][aria-label]").forEach((function(n, i) {
            if ("true" !== n.getAttribute("aria-hidden") && !0 !== n.hidden && "none" !== getComputedStyle(n).getPropertyValue("display")) {
            ...
            0 == a && "hidden" !== getComputedStyle(e).getPropertyValue("visibility") && "hidden" == getComputedStyle(n).getPropertyValue("visibility") ? t = !1 : a = !0
        })), t
        ...
        if (s.length > 0) s.forEach((function(e, t) {
            "true" !== e.getAttribute("aria-hidden") && !0 !== e.hidden && "none" !== getComputedStyle(e).getPropertyValue("display") && "none" !== getComputedStyle(e.parentNode).getPropertyValue("display") && (a = !0)
        }));
        else if (null !== i) {
            1 == i.querySelectorAll("input:not([type=image],[type=submit],[type=reset],[type=button],[type=hidden]), select, textarea, meter, output, progress").length && (i.getAttribute("for") ? i.getAttribute("for") == n && (a = !0) : a = !0)
        }
        return a
    }, wave.engine.fn.isLabeled = function(e, t) {
        return e.getAttribute("title") || e.getAttribute("aria-labelledby") || e.getAttribute("aria-label") || wave.engine.fn.isLabeledByTag(e, t)
        ...
        for (var t = []; e.nodeType === Node.ELEMENT_NODE;) {
            ...
            if (e.id) a += "#" + e.id;
                ...
                for (var n = e, i = []; null !== n && n.nodeType === Node.ELEMENT_NODE;) i.unshift(n.nodeName), n = wave.engine.fn.previousElementSibling(n);
        ...
        if (i) {
            if (wave.engine.overrides[t]) {
                for (var r in wave.engine.overrides[t])
                    if (void 0 !== wave.engine.iconList[r] && void 0 !== wave.engine.iconList[r].items[s]) {
                        delete wave.engine.iconList[r].items[s];
                        break
                    }
            } else if (wave.engine.overriddenby[t])
                for (var r in wave.engine.overriddenby[t])
                    if (void 0 !== wave.engine.iconList[r] && void 0 !== wave.engine.iconList[r].items[s]) return;
            if (wave.engine.iconList[t] || (wave.engine.iconList[t] = {
                    items: {}
                }), null == wave.engine.iconList[t].items[s]) {
                var l = "#" !== s && (0 == e.offsetWidth || 0 == e.offsetHeight || e.offsetTop < 0 || e.offsetLeft < 0);
                wave.engine.iconList[t].items[s] = "contrast" == t ? {
                    id: t,
                    description: i.title,
                    text: !1,
                    contrastdata: a,
        ...
        for (var a = e.childNodes, n = 0; n < a.length; n++)
            if (3 == a[n].nodeType && a[n].nodeValue.length > 0 && -1 == a[n].nodeValue.search(/^\s+$/i)) {
                t = !0;
                break
            } return "INPUT" == e.tagName && e.value.length > 0 && ["text", "button", "email", "number", "password", "reset", "submit", "tel", "url", "button"].includes(e.type) && (t = !0), "SELECT" == e.tagName && e.options.length > 0 && (t = !0), t
    }, wave.engine.fn.calculateContrast = function(e, t, a) {
        if (!e) {
            e = wave.page.body;
            t = this.getColor(e, "fg");
            if ("0" == (a = this.getColor(e, "bg"))[3]) "0" == (a = this.getColor(wave.page.querySelector("html"), "bg"))[3] && (a = ["255", "255", "255", "1", "rgb(255, 255, 255)"]);
            wave.engine.fn.elementHasText(e) && this.checkContrast(e, t, a)
        }
        for (var n = 0; e.children && n < e.children.length; n++) {
            var i = e.children[n];
            if ("wave5bottombar" !== i.getAttribute("id") && "SVG" !== i.tagName.toUpperCase() && "SCRIPT" !== i.tagName && "IMG" !== i.tagName && "STYLE" !== i.tagName && "NOSCRIPT" !== i.tagName.toUpperCase() && "VIDEO" !== i.tagName.toUpperCase() && "AUDIO" !== i.tagName.toUpperCase() && "OPTION" !== i.tagName.toUpperCase() && 1 != i.disabled && !0 !== i.hidden && "none" !== getComputedStyle(i).getPropertyValue("display") && "hidden" !== getComputedStyle(i).getPropertyValue("visibility") && "0px" !== getComputedStyle(i).getPropertyValue("font-size"))
                if (wave.engine.contrast[i.getAttribute("data-waveid")]) i.childNodes && wave.engine.fn.calculateContrast(i, t, a);
                else {
                    var s = this.getColor(i, "fg"),
                        o = this.getColor(i, "bg");
                    "0" == s[3] && (s = t), "0" == o[3] && (o = a), wave.engine.fn.elementHasText(i) && 1 == parseFloat(o[3]) ? s !== t || o !== a ? (this.checkContrast(i, s, o), wave.engine.fn.calculateContrast(i, s, o)) : wave.engine.fn.calculateContrast(i, s, o) : i.childNodes && wave.engine.fn.calculateContrast(i, s, o)
                }
        }
    }, wave.engine.fn.getColor = function(e, t) {
        var a = getComputedStyle(e);
        if ("fg" == t) {
            var n = a.color.replace(/[^\d,.]/g, "").split(",");
            return n[3] || (n[3] = "1"), n[4] = a.color, n
        }
        var i = a.backgroundColor.replace(/[^\d,.]/g, "").split(",");
        return i[3] || (i[3] = "1"), (a.backgroundImage.indexOf("linear-gradient") > -1 || a.opacity < 1 || "none" != a.filter && "blur(0px)" != a.filter) && (i[3] = ".1"), i[4] = a.backgroundColor, i
    }, wave.engine.fn.checkContrast = function(e, t, a) {
        var n = {},
            i = "",
            s = new wave.engine.fn.waveColorManager(t),
            o = new wave.engine.fn.waveColorManager(a),
            r = s.contrast(o).ratio;
        if (r) {
            var l = !1,
                c = !1;
            i = r + ":1", n.ratio = Number(r), n.contrastratio = i;
            var d = 72 * parseFloat(getComputedStyle(e).getPropertyValue("font-size")) / 96;
            if (r < 4.5 && (l = !0, (d >= 18 || d >= 14 && "bold" === e.style.fontWeight || d >= 14 && getComputedStyle(e).getPropertyValue("font-weight") >= 700) && (l = r < 3, c = !0)), l) {
                var u = wave.engine.fn.waveRGBToHex(t[4]),
                    g = wave.engine.fn.waveRGBToHex(a[4]),
                    m = [wave.engine.fn.Dec2(n.ratio), u, g, c];
    ...
    }, wave.engine.fn.waveColorManager = function(e) {
        ...
        void 0 === e[3] && (e[3] = 1), e = e.map((function(e) {
            return Math.round(e, 3)
        })), this.rgba = e
    }, wave.engine.fn.waveColorManager.prototype = {
        get rgb() {
        ...
        get luminance() {
            for (var e = this.rgba.slice(), t = 0; t < 3; t++) {
                var a = e[t];
                a = (a /= 255) < .03928 ? a / 12.92 : Math.pow((a + .055) / 1.055, 2.4), e[t] = a
            }
            return .2126 * e[0] + .7152 * e[1] + .0722 * e[2]
        },
        clone: function() {
            return new wave.engine.fn.waveColorManager(this.rgba)
        },
        overlayOn: function(e) {
            var t = this.clone(),
                a = this.alpha;
            if (a >= 1) return t;
            for (var n = 0; n < 3; n++) t.rgba[n] = t.rgba[n] * a + e.rgba[n] * e.rgba[3] * (1 - a);
            return t.rgba[3] = a + e.rgba[3] * (1 - a), t
        },
        contrast: function(e) {
            if (this.alpha >= 1) {
                e.alpha < 1 && (e = e.overlayOn(this));
                var t = this.luminance + .05,
                    a = e.luminance + .05,
                    n = t / a;
                return a > t && (n = 1 / n), {
                    ratio: n
        ...
        let t = e.indexOf(",") > -1 ? "," : " ",
            a = (+(e = e.substr(4).split(")")[0].split(t))[0]).toString(16),
            n = (+e[1]).toString(16),
            i = (+e[2]).toString(16);
        return 1 == a.length && (a = "0" + a), 1 == n.length && (n = "0" + n), 1 == i.length && (i = "0" + i), "#" + a + n + i
        ...
        alt_link_missing: {
            ...
            details: "An image without alternative text (missing alt attribute or an alt value that is null/empty or only space characters) or that is hidden is within a link that does not contain text or an image with alternative text.",
            resources: "",
            icon_order: "2",
            position: "after",
            page_rule: "0",
        ...
        image_title: {
            ...
            purpose: "The title attribute value for images that lack an alt attribute value will be presented to screen reader users. However, providing image content in the alt attribute typically provides better accessibility, and should be used in most cases. The title attribute will generate a mouse hover tooltip which may or may not be desired - this tooltip will not be presented to touch screen or keyboard users.",
        ...
        link_internal_broken: {
            ...
            purpose: "A link to jump to another position within the page assists users in navigating the web page, but only if the link target exists.",
            actions: "Ensure that the target for the link exists or remove the same-page link.",
        ...
        audio_video: {
            ...
            details: "An embedded QuickTime, Windows Media Player, or RealPlayer movie is present or a link is present to a file with one of the following extensions: 3gp, aif, aiff, asf, asx, avi, flv, m4a, m4p, mov, mp2, mp3, mp4, mpa, mpeg, mpeg2, mpg, mpv, ogg, ogv, qtl, ra, ram, smi, smil, wav, wax, webm, wma, wmv, wmp, or wmx.",
            resources: "",
            icon_order: "28",
            position: "after",
            page_rule: "0",
        ...
        underline: {
            ...
            details: "A &lt;u&gt; element or element with text-decoration:underline styles is present.",
            resources: "",
            icon_order: "41",
            position: "last",
            page_rule: "0",
...
wave.rules = {}, wave.rules.alt_missing = function(e) {
    wave.engine.fn.overrideby("alt_missing", ["alt_link_missing", "alt_map_missing", "alt_spacer_missing"]), e.querySelectorAll("img:not([alt])").forEach((function(e, t) {
        null != e.getAttribute("title") && 0 !== e.getAttribute("title").length || null != e.getAttribute("aria-label") && 0 !== e.getAttribute("aria-label").length || null != e.getAttribute("aria-labelledby") && 0 !== e.getAttribute("aria-labelledby").length || wave.engine.fn.addIcon(e, "alt_missing")
    }))
}, wave.rules.alt_link_missing = function(e) {
    wave.engine.fn.override("alt_link_missing", ["alt", "alt_missing", "alt_spacer_missing", "alt_null", "alt_spacer", "link_empty"]), e.querySelectorAll("a").forEach((function(e, t) {
        "wave5codetoggle" != e.parentNode.id && "none" != getComputedStyle(e).getPropertyValue("display") && (wave.engine.fn.hasTextContent(e) || e.querySelectorAll("img").forEach((function(e, t) {
            wave.engine.fn.addIcon(e, "alt_link_missing")
        })))
    }))
}, wave.rules.alt_spacer_missing = function(e) {
    wave.engine.fn.override("alt_spacer_missing", ["alt_missing"]), wave.engine.fn.overrideby("alt_spacer_missing", ["alt_link_missing"]), e.querySelectorAll("img:not([alt])").forEach((function(e, t) {
        var a = e.height,
            n = e.width,
            i = e.getAttribute("src");
        (i && -1 !== (i = (i = (i = e.getAttribute("src").split("/"))[i.length - 1].split("\\"))[i.length - 1]).search(/^(spacer|space|blank)\..+$/i) || null !== a && a > 0 && a <= 3 || null !== n && n > 0 && n <= 3) && wave.engine.fn.addIcon(e, "alt_spacer_missing")
    }))
}, wave.rules.alt_input_missing = function(e) {
    e.querySelectorAll("input[type=image]").forEach((function(e, t) {
        var a = e.getAttribute("alt");
        a && -1 === a.search(/^ *$/) || wave.engine.fn.addIcon(e, "alt_input_missing")
    }))
    ...
    wave.engine.fn.override("alt_map_missing", ["alt_missing"]), e.querySelectorAll("img[usemap]:not([alt])").forEach((function(e, t) {
        wave.engine.fn.addIcon(e, "alt_map_missing")
    }))
}, wave.rules.longdesc_invalid = function(e) {
    wave.engine.fn.override("longdesc_invalid", ["longdesc"]), e.querySelectorAll("[longdesc]").forEach((function(e, t) {
        var a = new RegExp("(([a-zA-Z0-9]|\\|/))+"),
            n = new RegExp("(.html|.htm)$"),
            i = new RegExp("(.jpg|.gif|.png)$"),
            s = e.getAttribute("longdesc");
        a.test(s) && (0 == s.length || i.test(s) || -1 != s.indexOf(" ") && !n.test(s)) && wave.engine.fn.addIcon(e, "longdesc_invalid", s)
    }))
}, wave.rules.select_missing_label = function(e) {
    wave.engine.fn.override("select", ["label_missing"]), e.querySelectorAll("select").forEach((function(t, a) {
        t.options.length > 0 && !wave.engine.fn.isLabeled(t, e) && t.options[t.selectedIndex].text.length > 0 && wave.engine.fn.addIcon(t, "select_missing_label")
    }))
}, wave.rules.label_missing = function(e) {
    wave.engine.fn.overrideby("label_missing", ["select_missing_label"]), e.querySelectorAll("input:not([type=image],[type=submit],[type=Submit],[type=SUBMIT],[type=reset],[type=Reset],[type=button],[type=hidden],[type=HIDDEN]), select, textarea").forEach((function(t, a) {
        wave.engine.fn.isLabeled(t, e) || wave.engine.fn.addIcon(t, "label_missing")
    }))
}, wave.rules.label_empty = function(e) {
    wave.engine.fn.override("label_empty", ["label", "label_orphaned"]), e.querySelectorAll("label").forEach((function(t, a) {
        var n = t.querySelectorAll("input, select, textarea, meter, output, progress"),
            i = t.getAttribute("for");
        (!wave.engine.fn.hasTextContent(t) || (n.length > 0 && wave.engine.fn.isLabeled(n[0], e) || null != i) && !wave.engine.fn.hasTextContent(t)) && wave.engine.fn.addIcon(t, "label_empty")
    }))
}, wave.rules.label_multiple = function(e) {
    e.querySelectorAll("input:not([type=image],[type=submit],[type=reset],[type=button],[type=hidden]), select, textarea").forEach((function(t, a) {
        var n = t.getAttribute("id"),
            i = t.closest("label"),
            s = null != i ? i.length : 0,
            o = null != n ? e.querySelectorAll("label[for='" + n + "']") : null;
        o && (o.length > 1 || 1 == s && 1 == o.length && i[0] !== o[0]) && wave.engine.fn.addIcon(t, "label_multiple")
    }))
    ...
    e.querySelectorAll("html:not([lang])").forEach((function(e, t) {
        ...
        i && ("HTML" == e.nodeName ? wave.engine.fn.addPagewideIcon("language_missing", s) : wave.engine.fn.addIcon(e, "language_missing", s))
    }))
    ...
    e.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((function(e, t) {
        wave.engine.fn.hasTextContent(e) || wave.engine.fn.addIcon(e, "heading_empty")
    }))
}, wave.rules.button_empty = function(e) {
    wave.engine.fn.overrideby("button_empty", ["alt_link_missing", "aria_label"]), e.querySelectorAll("button:not([type='reset'], [role='button']").forEach((function(e, t) {
        wave.engine.fn.hasTextContent(e) || 0 != e.title.length || "-1" === e.getAttribute("tabindex") || wave.engine.fn.addIcon(e, "button_empty")
    })), e.querySelectorAll("input[type='submit'], input[type='button'], input[type='reset']").forEach((function(e, t) {
        var a = e.getAttribute("value");
        (!a || a && 0 === a.length) && 0 == e.title.length && "-1" !== e.getAttribute("tabindex") && wave.engine.fn.addIcon(e, "button_empty")
    }))
}, wave.rules.link_empty = function(e) {
    wave.engine.fn.overrideby("link_empty", ["alt_link_missing", "aria_label"]), e.querySelectorAll("a[href]").forEach((function(e, t) {
        "wave5codetoggle" != e.parentNode.id && (0 != e.querySelectorAll("img").length || wave.engine.fn.hasTextContent(e) || 0 != e.title.length || "-1" === e.getAttribute("tabindex") || wave.engine.fn.addIcon(e, "link_empty"))
    }))
    ...
    e.querySelectorAll("a[href^='#']").forEach((function(t, a) {
        ...
        if (-1 !== t.textContent.trim().search(/^(skip|jump).*/gi)) {
            if ("none" == getComputedStyle(t).getPropertyValue("display") || "hidden" == getComputedStyle(t).getPropertyValue("visibility")) n = !0;
            else {
                var s = i.replace("#", "");
                0 === e.querySelectorAll('[id="' + s + '"]').length && 0 === e.querySelectorAll('a[name="' + s + '"]').length && (n = !0)
            }
    ...
    e.querySelectorAll("p, span, div, td, th, dd, ul, ol, h1, h2, h3, h4, h5, h6, blink").forEach((function(e, t) {
    ...
    for (var t in wave.engine.fn.calculateContrast(), wave.engine.contrast) wave.engine.fn.addIcon(wave.engine.contrast[t].element, "contrast", wave.engine.contrast[t].contrastData)
}, wave.rules.alt_redundant = function(e) {
    wave.engine.fn.override("alt_redundant", ["alt"]), wave.engine.fn.overrideby("alt_redundant", ["alt_suspicious"]), e.querySelectorAll("img:not([alt=''])[alt]").forEach((function(e, t) {
        var a = e.getAttribute("alt");
        if (!(a && a.length > 0 && -1 !== a.search(/^ +$/i) || a && a.length < 3)) {
            for (var n = "", i = e; n.length < 15;)
                if (i.nodeType == i.TEXT_NODE && (n += i.nodeValue.toLowerCase()), i.lastChild) i = i.lastChild;
            ...
            var s = "";
            for (i = e; s.length < 15;)
                if (i.nodeType == i.TEXT_NODE && (s += i.nodeValue.toLowerCase()), i.firstChild) i = i.firstChild;
                else if (i.nextSibling) i = i.nextSibling;
            else {
                if (!i.parentNode || !i.parentNode.nextSibling) break;
                i = i.parentNode.nextSibling
            }
            n = n.substring(n.length - 15 - a.toLowerCase().length, n.length - 1), s = s.substring(0, 14 + a.toLowerCase().length), -1 == n.indexOf(a.toLowerCase()) && -1 == s.indexOf(a.toLowerCase()) || wave.engine.fn.addIcon(e, "alt_redundant", a)
        }
    }))
}, wave.rules.alt_suspicious = function(e) {
    wave.engine.fn.override("alt_suspicious", ["alt", "alt_redundant"]), e.querySelectorAll("img[alt]").forEach((function(e, t) {
        var a = e.getAttribute("alt");
        a && a.length > 0 && (-1 !== a.search(/^(image|graphic|photo|photograph|drawing|painting|artwork|logo|bullet|button|arrow|more|spacer|blank|chart|table|diagram|graph|\*)$/i) || -1 !== a.search(/(^(graphic of|bullet|image of).*)|(.*(image|graphic)$)|(^ +$)|.*(\.png|\.gif|\.jpeg|\.jpg|\.svg|\.bmp)$/i)) && wave.engine.fn.addIcon(e, "alt_suspicious", a)
    }))
}, wave.rules.alt_duplicate = function(e) {
    wave.engine.fn.override("alt_duplicate", ["alt", "alt_missing"]);
    for (var t = e.querySelectorAll("img, input[type='image']"), a = 0; a < t.length; a++)
        if (t[a].hasAttribute("alt") && "" != t[a].getAttribute("alt") && t[a].offsetWidth > 0 && t[a].offsetHeight > 0)
            for (var n = a + 1; n <= a + 2 && n < t.length; n++)
                if (t[n].hasAttribute("alt") && t[n].getAttribute("alt").toLowerCase() == t[a].getAttribute("alt").toLowerCase() && t[n].hasAttribute("src") && t[n].getAttribute("src").toLowerCase() !== t[a].getAttribute("src").toLowerCase() && t[n].offsetWidth > 0 && t[n].offsetHeight > 0) {
                    wave.engine.fn.addIcon(t[n], "alt_duplicate", t[n].getAttribute("alt"));
                    break
                }
    ...
    wave.engine.fn.overrideby("label_orphaned", ["label_empty"]), e.querySelectorAll("label").forEach((function(t, a) {
        var n = t.querySelectorAll("input, select, textarea, meter, output, progress"),
            i = t.getAttribute("for")?.replace("'", "");
        (0 == n.length && !i || i && 0 == e.querySelectorAll("input[id='" + i + "'], select[id='" + i + "'], textarea[id='" + i + "'], meter[id='" + i + "'], output[id='" + i + "'], progress[id='" + i + "']").length) && wave.engine.fn.addIcon(t, "label_orphaned")
    }))
}, wave.rules.fieldset_missing = function(e) {
    var t = {};
    for (var a in e.querySelectorAll("input[name][type=checkbox], input[name][type=radio]").forEach((function(e, a) {
        ...
        if (2 == t[a].length) {
            ...
            if (i && i.length > 0) wave.engine.fn.addIcon(i.get(0), "fieldset_missing");
            else if (n) {
    ...
    wave.engine.fn.override("legend_missing", ["fieldset"]), e.querySelectorAll("fieldset").forEach((function(e, t) {
        0 === e.querySelectorAll("legend").length && wave.engine.fn.addIcon(e, "legend_missing")
    }))
}, wave.rules.label_title = function(e) {
    e.querySelectorAll("input[title]:not([type=image],[type=submit],[type=Submit],[type=SUBMIT],[type=reset],[type=Reset],[type=button],[type=hidden],[type=HIDDEN]), select[title], textarea[title]").forEach((function(t, a) {
        !t.getAttribute("aria-label") && !t.getAttribute("aria-labelledby") && !wave.engine.fn.isLabeledByTag(t, e) && t.getAttribute("title").length > 0 && wave.engine.fn.addIcon(t, "label_title")
    }))
}, wave.rules.heading_missing = function(e) {
    wave.engine.fn.override("heading_missing", ["h1_missing"]), 0 === e.querySelectorAll("h1, h2, h3, h4, h5, h6").length && wave.engine.fn.addPagewideIcon("heading_missing")
}, wave.rules.region_missing = function(e) {
    0 === e.querySelectorAll("header, nav, main, footer, aside, [role=banner], [role=navigation], [role=main], [role=contentinfo], [role=region]").length && wave.engine.fn.addPagewideIcon("region_missing")
}, wave.rules.h1_missing = function(e) {
    wave.engine.fn.overrideby("h1_missing", ["heading_missing"]), 0 === e.querySelectorAll("h1").length && wave.engine.fn.addPagewideIcon("h1_missing")
}, wave.rules.heading_skipped = function(e) {
    var t = 0,
        a = 0;
    e.querySelectorAll("h1, h2, h3, h4, h5, h6").forEach((function(e, n) {
        a = Number(e.nodeName[1]), 0 != t && t - a < -1 && wave.engine.fn.addIcon(e, "heading_skipped"), t = a
    }))
    ...
    e.querySelectorAll("p").forEach((function(e, t) {
        ...
        e.textContent.length > 0 && e.textContent.length < 50 && (a > 20 || a > 16 && (getComputedStyle(e).getPropertyValue("font-weight") >= 700 || "bold" == e.style.fontWeight || "italic" == getComputedStyle(e).getPropertyValue("font-style"))) && wave.engine.fn.addIcon(e, "heading_possible")
    }))
}, wave.rules.table_caption_possible = function(e) {
    e.querySelectorAll("table").forEach((function(e, t) {
        if (e.querySelectorAll("th").length > 0 && 0 == e.querySelectorAll("caption").length) {
            var a = e.previousElementSibling,
                n = e.querySelectorAll("td");
            if (n.length > 0) {
                var i = n[0].getAttribute("colspan");
                i && i >= 3 ? wave.engine.fn.addIcon(n[0], "table_caption_possible") : a && "P" == a.nodeName && (a.textContent.length < 50 || a.textContent.length < 100 && getComputedStyle(a).getPropertyValue("font-weight") >= 700 && "center" == getComputedStyle(a).getPropertyValue("text-align")) && wave.engine.fn.addIcon(a, "table_caption_possible")
            }
        }
    }))
    ...
    e.querySelectorAll("p").forEach((function(e, a) {
        ...
        if (n != t && (t = n, 0 == n.search(/^(\*\s|\-\s|\-|1\.|a\.|1\)|a\))/))) {
            0 == e.nextElementSibling?.textContent?.trim().search(/^(\*\s|\-\s|\-|2\.|b\.|2\)|b\))/) && wave.engine.fn.addIcon(e, "list_possible");
            var i = e.innerHTML.split(/<br\s*.*?\/?>[\n\r]*\s*/gi, 2);
            i[1] && 0 == i[1].trim().search(/^(\*\s|\-\s|\-|2\.|b\.|2\)|b\))/) && wave.engine.fn.addIcon(e, "list_possible")
        }
    }))
}, wave.rules.link_internal_broken = function(e) {
    wave.engine.fn.overrideby("link_internal_broken", ["link_skip_broken"]), e.querySelectorAll("a[href^='#']:not(a[href^='#/'],[href^='#top']").forEach((function(t, a) {
        var n = t.getAttribute("href"),
            i = n.replace("#", "");
        n[0] && "#" != n && 0 === e.querySelectorAll('[id="' + i + '"]').length && 0 === e.querySelectorAll('a[name="' + i + '"]').length && wave.engine.fn.addIcon(t, "link_internal_broken")
    }))
}, wave.rules.link_suspicious = function(e) {
    e.querySelectorAll("a[href]").forEach((function(e, t) {
        var a = e.textContent;
        !a || -1 === a.search(/(click)/i) && -1 === a.search(/^(here|more|more\.\.\.|details|more details|link|this page|continue|continue reading|read more|button)$/i) || wave.engine.fn.addIcon(e, "link_suspicious")
    }))
}, wave.rules.link_redundant = function(e) {
    var t = null;
    e.querySelectorAll("a[href]:not(a[href^='#'])").forEach((function(e, a) {
        t && t.search(/^\s*$/i) && t == e.getAttribute("href") ? (t = e.getAttribute("href"), wave.engine.fn.addIcon(e, "link_redundant")) : t = e.getAttribute("href")
    }))
}, wave.rules.link_word = function(e) {
    e.querySelectorAll("a[href]").forEach((function(e, t) {
        var a = e.getAttribute("href");
        a && -1 !== a.search(/(\.doc|\.docx)$/i) && wave.engine.fn.addIcon(e, "link_word")
    }))
}, wave.rules.link_excel = function(e) {
    e.querySelectorAll("a[href]").forEach((function(e, t) {
        var a = e.getAttribute("href");
        a && -1 !== a.search(/(\.xls|\.xlsx)$/i) && wave.engine.fn.addIcon(e, "link_excel")
    }))
}, wave.rules.link_powerpoint = function(e) {
    e.querySelectorAll("a[href]").forEach((function(e, t) {
        var a = e.getAttribute("href");
        a && -1 !== a.search(/(\.ppt|\.pptx|\.pps|\.ppsx)$/i) && wave.engine.fn.addIcon(e, "link_powerpoint")
    }))
    ...
    e.querySelectorAll("a[href]").forEach((function(e, t) {
        var a = e.getAttribute("href");
        a && -1 !== a.search(/(\.rtf|\.wpd|\.ods|\.odt|\.odp|\.sxw|\.sxc|\.sxd|\.sxi|\.pages|\.key)$/i) && wave.engine.fn.addIcon(e, "link_document")
    }))
}, wave.rules.audio_video = function(e) {
    wave.engine.fn.override("audio_video", ["plugin"]), e.querySelectorAll("embed").forEach((function(e, t) {
        var a = e.getAttribute("src");
        a && -1 !== a.search(/(\.mov|\.asx|\.wvx|\.wax|\.wmv|\.wma|\.ram|\.rpm|\.ra|\.rm)$/i) && wave.engine.fn.addIcon(e, "audio_video")
    })), e.querySelectorAll("embed[type='video/quicktime'], object[classid='clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B'], embed[type='application/x-mplayer2'], object[classid='clsid:22D6F312-B0F6-11D0-94AB-0080C74C7E95'], object[classid='clsid:6BF52A52-394A-11d3-B153-00C04F79FAA6'], embed[type='audio/x-pn-realaudio-plugin'], object[classid='clsid:CFCDAA03-8BE4-11cf-B84B-0020AFBBCCFA']").forEach((function(e, t) {
        wave.engine.fn.addIcon(e, "audio_video")
    })), e.querySelectorAll("a[href]").forEach((function(e, t) {
        var a = e.getAttribute("href");
        a && -1 !== a.search(/(\.3gp|\.aif|\.aiff|\.asf|\.asx|\.avi|\.flv|\.m4a|\.m4p|\.mov|\.mp2|\.mp3|\.mp4|\.mpa|\.mpeg|\.mpeg2|\.mpg|\.mpv|\.ogg|\.ogv|\.qtl|\.ra|\.ram|\.smi|\.smil|\.wav|\.wax|\.webm|\.wma|\.wmp|\.wmx)$/i) && wave.engine.fn.addIcon(e, "audio_video")
    }))
}, wave.rules.flash = function(e) {
    wave.engine.fn.override("flash", ["plugin"]), e.querySelectorAll("embed[src], embed[type='application/x-shockwave-flash']").forEach((function(e, t) {
        var a = e.getAttribute("src");
        if (a && -1 !== a.search(/(\.swf)$/i)) wave.engine.fn.addIcon(e, "flash");
        else {
            var n = e.getAttribute("type");
            n && "application/x-shockwave-flash" === n && wave.engine.fn.addIcon(e, "flash")
        }
    })), e.querySelectorAll("object[type='application/x-shockwave-flash'], object[classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000'], object[classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000']").forEach((function(e, t) {
    ...
    wave.engine.fn.overrideby("plugin", ["flash", "audio_video"]), e.querySelectorAll("object,embed").forEach((function(e, t) {
    ...
    e.querySelectorAll("[onmouseover],[onclick],[ondblclick]").forEach((function(e, t) {
        ...
        a && !n || s ? wave.engine.fn.addIcon(e, "event_handler") : i && "a" !== o && "input" !== o && "button" !== o && "select" !== o && "option" !== o && "textarea" !== o && (r && "0" === r || wave.engine.fn.addIcon(e, "event_handler"))
    }))
    ...
    e.querySelectorAll("[tabindex]").forEach((function(e, t) {
        ...
        "NaN" !== Number(a) && Number(a) > 0 && wave.engine.fn.addIcon(e, "tabindex")
    }))
}, wave.rules.text_small = function(e) {
    e.querySelectorAll("p, span:not(.wavehidden), div:not(#wave5spinner), td, th, dd, ul, ol, h1, h2, h3, h4, h5, h6, font, i").forEach((function(e, t) {
        var a = parseInt(getComputedStyle(e).getPropertyValue("font-size"), 10);
        wave.engine.fn.elementHasText(e) && a <= 10 && wave.engine.fn.addIcon(e, "text_small")
    }))
}, wave.rules.text_justified = function(e) {
    e.querySelectorAll("p, div, td").forEach((function(e, t) {
        "justify" == getComputedStyle(e).getPropertyValue("text-align") && wave.engine.fn.addIcon(e, "text_justified")
    }))
}, wave.rules.underline = function(e) {
    e.querySelectorAll("p, span, div, td, th, dd, li, u").forEach((function(e, t) {
        "U" == e.nodeName && "A" != e.parentNode.nodeName ? wave.engine.fn.addIcon(e, "underline") : "underline" != getComputedStyle(e).getPropertyValue("text-decoration") && "underline" != getComputedStyle(e).getPropertyValue("text-decoration-line") || wave.engine.fn.addIcon(e, "underline")
    }))
}, wave.rules.title_redundant = function(e) {
    e.querySelectorAll("[title]:not(img)").forEach((function(e, t) {
        var a = e.getAttribute("title");
        a && a.length > 0 && e.textContent == a && wave.engine.fn.addIcon(e, "title_redundant")
    })), e.querySelectorAll("img[title]").forEach((function(e, t) {
        var a = e.getAttribute("title");
        a && a.length > 0 && e.getAttribute("alt") == a && wave.engine.fn.addIcon(e, "title_redundant")
    }))
}, wave.rules.alt = function(e) {
    wave.engine.fn.overrideby("alt", ["alt_redundant", "alt_duplicate", "alt_link", "alt_link_missing", "alt_spacer", "alt_map", "alt_suspicious"]), e.querySelectorAll("img[alt]").forEach((function(e, t) {
        var a = e.getAttribute("alt");
        a && a.length > 0 && a.length <= 100 && wave.engine.fn.addIcon(e, "alt", a)
    }))
}, wave.rules.alt_null = function(e) {
    wave.engine.fn.overrideby("alt_null", ["alt_link_missing", "alt_spacer"]), e.querySelectorAll("img[alt='']").forEach((function(e, t) {
        wave.engine.fn.addIcon(e, "alt_null")
    }))
}, wave.rules.alt_spacer = function(e) {
    wave.engine.fn.override("alt_spacer", ["alt", "alt_missing", "alt_null"]), wave.engine.fn.overrideby("alt_spacer", ["alt_link_missing"]), e.querySelectorAll("img[alt='']").forEach((function(e, t) {
        if ("" == e.getAttribute("alt")) {
            var a = e.height,
                n = e.width,
                i = e.getAttribute("src");
            (i && -1 !== (i = (i = (i = e.getAttribute("src").split("/"))[i.length - 1].split("\\"))[i.length - 1]).search(/^(spacer|space|blank)\..+$/i) || null !== a && a > 0 && a <= 3 || null !== n && n > 0 && n <= 3) && wave.engine.fn.addIcon(e, "alt_spacer")
        }
    }))
}, wave.rules.alt_link = function(e) {
    wave.engine.fn.override("alt_link", ["alt", "alt_missing"]), e.querySelectorAll("a").forEach((function(e, t) {
        if ("wave5codetoggle" != e.parentNode.id && wave.engine.fn.hasTextContent(e)) {
            var a = null;
            e.querySelectorAll("img[alt]").forEach((function(e, t) {
            ...
            })), null !== a && wave.engine.fn.addIcon(a, "alt_link", a.getAttribute("alt"))
        }
    }))
    ...
    wave.engine.fn.override("alt_map", ["alt", "alt_missing"]), e.querySelectorAll("img[alt][usemap]").forEach((function(e, t) {
        var a = e.getAttribute("alt");
        a && a.length > 0 && -1 === a.search(/^ +$/i) ? wave.engine.fn.addIcon(e, "alt_map", a) : wave.engine.fn.addIcon(e, "alt_map")
    }))
    ...
    wave.engine.fn.overrideby("longdesc", ["longdesc_invalid"]), e.querySelectorAll("[longdesc]").forEach((function(e, t) {
    ...
    wave.engine.fn.overrideby("label", ["label_empty"]), e.querySelectorAll("label").forEach((function(t, a) {
        if (wave.engine.fn.hasTextContent(t)) {
            var n = t.querySelectorAll("input, select, textarea, meter, output, progress"),
                i = t.getAttribute("for")?.replace("'", "");
            (n.length > 0 && wave.engine.fn.isLabeled(n[0], e) || i && 1 == e.querySelectorAll("input[id='" + i + "'], select[id='" + i + "'], textarea[id='" + i + "'], meter[id='" + i + "'], output[id='" + i + "'], progress[id='" + i + "']").length) && wave.engine.fn.addIcon(t, "label")
        }
    }))
}, wave.rules.fieldset = function(e) {
    wave.engine.fn.overrideby("fieldset", ["legend_missing"]), e.querySelectorAll("fieldset").forEach((function(e, t) {
        var a = e.querySelector("legend");
        wave.engine.fn.addIcon(null != a ? a : e, "fieldset")
    }))
}, wave.rules.link_skip = function(e) {
    wave.engine.fn.overrideby("link_skip", ["link_skip_broken"]), e.querySelectorAll("a[href]").forEach((function(t, a) {
        var n = t.getAttribute("href");
        if (n && 0 != n.length && "#" == n[0] && "#" != n) {
            var i = n.replace("#", ""); - 1 !== t.textContent.trim().search(/^(skip|jump).*/gi) && n && (e.querySelectorAll(n).length > 0 || e.querySelectorAll('[name="' + i + '"]').length > 0) && wave.engine.fn.addIcon(t, "link_skip")
        }
    }))
    ...
    e.querySelectorAll("a[href]").forEach((function(t, a) {
        var n = t.getAttribute("href");
        if (n && 0 != n.length && "#" == n[0] && "#" != n) {
            if (!0 !== t.hidden && "none" != getComputedStyle(t).getPropertyValue("display")) var i = n.replace("#", ""); - 1 !== t.textContent.trim().search(/^(skip|jump).*/gi) && (n && e.querySelectorAll(n).length > 0 && wave.engine.fn.addIcon(e.querySelectorAll(n)[0], "link_skip_target"), e.querySelectorAll('[name="' + i + '"]').length > 0 && wave.engine.fn.addIcon(e.querySelector('[name="' + i + '"]'), "link_skip_target"))
        }
    }))
}, wave.rules.lang = function(e) {
    wave.engine.fn.overrideby("lang", ["language_missing"]), e.querySelectorAll("[lang]").forEach((function(e, t) {
        var a = e.getAttribute("lang");
        a && a.length > 0 && ("HTML" == e.nodeName ? wave.engine.fn.addPagewideIcon("lang", a) : wave.engine.fn.addIcon(e, "lang", a))
    }))
}, wave.rules.table_layout = function(e) {
    e.querySelectorAll("table").forEach((function(e, t) {
        var a = !1;
        if (e.querySelectorAll("th").forEach((function(t, n) {
                var i = t.closest("table");
                20 != e.compareDocumentPosition(i) && (a = !0)
            })), 0 == a) {
            var n = e.querySelectorAll("td");
            n.length > 0 ? wave.engine.fn.addIcon(n[0], "table_layout") : wave.engine.fn.addIcon(e, "table_layout")
        }
    }))
    ...
    e.querySelectorAll("header,[role='banner']").forEach((function(e, t) {
        ...
        if (!a || 20 == e.compareDocumentPosition(a)) {
            ...
            n && 20 != e.compareDocumentPosition(n) || wave.engine.fn.addIcon(e, "header")
        }
    }))
    ...
    e.querySelectorAll("footer,[role='contentinfo']").forEach((function(e, t) {
        ...
        if (!a || 20 == e.compareDocumentPosition(a)) {
            var n = e.closest("section");
            n && 20 != e.compareDocumentPosition(n) || wave.engine.fn.addIcon(e, "footer")
        }
    }))
    ...
    wave.engine.fn.override("region", ["region_missing"]), e.querySelectorAll("[role='region']:not(#wave5bottombar)").forEach((function(e, t) {
    ...
    e.querySelectorAll("iframe[src*='youtube.com'],iframe[data-src*='youtube.com'],iframe[src*='youtube-nocookie.com'],iframe[data-src*='youtube-nocookie.com']").forEach((function(e, t) {
        wave.engine.fn.addIcon(e, "youtube_video")
    })), e.querySelectorAll("a[href*='youtube.com']").forEach((function(e, t) {
        var a = e.getAttribute("href").split(/(vi\/|v%3D|v=|\/v\/|youtu\.be\/|\/embed\/)/),
            n = void 0 !== a[2] ? a[2].split(/[^0-9a-z_\-]/i)[0] : a[0];
        n.includes("youtube") || n.includes("user") || wave.engine.fn.addIcon(e, "youtube_video")
    }))
    ...
    var t = ["aria-label", "aria-labelledby", "aria-describedby", "aria-live", "aria-hidden", "aria-expanded", "aria-haspopup"],
        a = ["banner", "complementary", "contentinfo", "main", "navigation", "search", "alert", "alertdialog", "menu", "menubar", "button", "region"];
    e.querySelectorAll("*:not(#wave5bottombar)").forEach((function(e, n) {
        for (var i = "", s = 0; s < e.attributes.length; s++)(-1 !== e.attributes[s].name.search(/^aria-/i) || e.getAttribute("role")) && (-1 !== e.attributes[s].name.search(/^aria-/i) && t.indexOf(e.attributes[s].name) < 0 || "role" == e.attributes[s].name && a.indexOf(e.attributes[s].value) < 0) && (i += e.attributes[s].name + '="' + e.attributes[s].value + '", ');
        "" !== i && wave.engine.fn.addIcon(e, "aria", i.slice(0, -2))
    }))
}, wave.rules.aria_label = function(e) {
    wave.engine.fn.override("aria_label", ["link_empty", "button_empty"]), e.querySelectorAll("[aria-labelledby]:not(#wave5bottombar), [aria-label]:not(#wave5bottombar)").forEach((function(e, t) {
    ...
    wave.engine.fn.override("aria_reference_broken", ["aria_label", "aria_describedby"]);
    var t = !1;
    e.querySelectorAll("*[aria-labelledby], *[aria-describedby]").forEach((function(a, n) {
        t = !1;
    ...
    e.querySelectorAll("[tabindex]").forEach((function(e, t) {
        var a = e.getAttribute("tabindex");
        "NaN" !== Number(a) && Number(a) < 1 && wave.engine.fn.addIcon(e, "aria_tabindex", "tabindex=" + a)
    }))
}, wave.rules.aria_live_region = function(e) {
    e.querySelectorAll("[role='alert'],[role='alertdialog'],[aria-live]").forEach((function(e, t) {
        wave.engine.fn.addIcon(e, "aria_live_region")
    }))
}, wave.rules.aria_menu_broken = function(e) {
    e.querySelectorAll("[role='menu']:not([aria-busy='true']),[role='menubar']:not([aria-busy='true'])").forEach((function(e, t) {
        0 === e.querySelectorAll("[role='menuitem'], [role='menuitemcheckbox'], [role='menuitemradio']").length && wave.engine.fn.addIcon(e, "aria_menu_broken")
    }))
}, wave.rules.aria_menu = function(e) {
    wave.engine.fn.overrideby("aria_menu", ["aria_menu_broken"]), e.querySelectorAll("[role='menu'],[role='menubar']").forEach((function(e, t) {
    ...
    e.querySelectorAll("[aria-expanded='true'],[aria-expanded='false']").forEach((function(e, t) {
...
}, "extension" == waveconfig.platform ? wave.init() : document.addEventListener("DOMContentLoaded", (function() {
    wave.init()
    ...
    var t, a, n = "extension" == waveconfig.platform ? document.body : document.getElementById("report");
    ...
    if (waveconfig.debug && console.log("Start buildOutline."), void 0 === e) {
        waveconfig.debug && console.log("outlineHtml undefined. Start buildOutlineHtml");
        e = wave.tabsengine.buildOutlineHtml();
        "extension" == waveconfig.platform ? (waveconfig.debug && console.log("Passing outline to handleOutlineData"), reportPageMessaging.fn.setupAndDispatchEvent("handleOutlineData", e)) : wave.tabs.handleOutlineData(e)
    }
}, wave.tabsengine.buildOutlineHtml = function() {
    var e = "",
        t = ["H1", "H2", "H3", "H4", "H5", "H6"],
        a = ["HEADER", "NAV", "MAIN", "ASIDE", "FOOTER", "SEARCH"],
        n = ["BANNER", "SEARCH", "NAVIGATION", "COMPLEMENTARY", "MAIN", "CONTENTINFO", "REGION"];
    ...
    for (var s = [], o = Object.keys(wave.report.iconlist.structure.items), r = [], l = 0; l < o.length; l++)
        for (var c = 0; c < wave.report.iconlist.structure.items[o[l]].xpaths.length; c++) r.push(wave.report.iconlist.structure.items[o[l]].xpaths[c]);
    for (var d in wave.report.things.allTagsCached)
        if (wave.report.things.allTagsCached.hasOwnProperty(d) && r.indexOf(d) > -1) {
            var u = wave.report.things.allTagsCached[d],
                g = u.getAttribute("role");
            g = null !== g ? g.toUpperCase() : g;
            var m, p = u.tagName.toUpperCase(),
                h = {
                    HEADER: "Header",
                    NAV: "Navigation",
                    MAIN: "Main",
            ...
            if (m = h.hasOwnProperty(p) ? h[p] : i(g), s.length > 0)
                for (l = 0; l < s.length; l++) - 1 == d.indexOf(s[l]) && (e += "</div>", s.splice(l, 1), l--);
            if (wave.tabsengine.isHeaderTag(t, p)) e += "<div class='outline_" + p + "' data-reportxpath-heading='" + d + "'>" + wave.tabsengine.getAccName(u, !1, !1, !1, !0) + "</div>";
            (wave.tabsengine.isAriaLandmark(a, p) || n.indexOf(g) > -1) && (s.push(d), e += "<div style='border: 1px solid #0050C7; padding: 10px; margin-bottom: 1px;'>", e += "<div class='outline_aria' data-reportxpath-heading='" + d + "'>" + m + "</div>")
        } return e
    ...
    if (wave.iconbox.hide(), !0 === e) {
        t.querySelectorAll("table, tbody, td, tfoot, th, thead, tr").forEach((function(e) {
            e.style.display = "", e.style.textAlign = ""
        }));
        var a = t.styleSheets;
        for (let e = 0; e < a.length; e++) a[e].disabled = !1;
        t.querySelectorAll("[wave5style]").forEach((function(e) {
            e.setAttribute("style", e.getAttribute("wave5style")), e.removeAttribute("wave5style")
        }))
    } else if (!1 === e) {
        t.querySelectorAll("svg:not(.wave5icon), img[src$='.svg']:not(.wave5icon)").forEach((function(e) {
            e.setAttribute("width", e.clientWidth), e.setAttribute("height", e.clientHeight)
        }));
        a = t.styleSheets;
        for (let e = 0; e < a.length; e++)(null == a[e].href || null != a[e].href && -1 == a[e].href.indexOf("chrome-extension://") && -1 == a[e].href.indexOf("moz-extension://")) && (a[e].disabled = !0);
        t.querySelectorAll("[style]:not(html,#wave_sidebar_container, #wave5topbar, #wave5bottombar, #wave5bottombar #wave5spinner, .wave5icon, .wave5text, .wave5text a, .waveorder)").forEach((function(e) {
            e.setAttribute("wave5style", e.getAttribute("style")), e.removeAttribute("style")
        })), t.querySelectorAll("table, tbody, td, tfoot, th, thead, tr").forEach((function(e) {
            e.style.display = "block", e.style.textAlign = "left"
        }))
    }
    1 == wave.tabsengine.navOrderVisible && wave.tabsengine.showNavOrder("show", wave.tabsengine.WaveIconsVisible), wave.report.things.updateTopbar()
}, wave.tabsengine.toggleSaturation = function(e) {
    "desaturate" == e ? document.documentElement.style.setProperty("filter", "grayscale(100%)") : document.documentElement.style.setProperty("filter", "")
}, wave.tabsengine.showNavOrder = function(e, t) {
    if (wave.iconbox.hide(), "show" != e || null == t || t == wave.tabsengine.WaveIconsVisible) {
        var a = "extension" == waveconfig.platform ? document : document.getElementById("report").contentWindow.document;
        if ("show" == e) {
            null != a.getElementById("waveordercontainer") && a.getElementById("waveordercontainer").remove(), wave.tabsengine.navOrderVisible = !0;
            var n = wave.tabsengine.buildNavItems(a);
            "extension" == waveconfig.platform ? (waveconfig.debug, reportPageMessaging.fn.setupAndDispatchEvent("handleNavData", n)) : wave.tabs.handleNavData(n)
        } else wave.tabsengine.navOrderVisible = !1, wave.tabsengine.showWaveIcons(!0), null != a.getElementById("waveordercontainer") && a.getElementById("waveordercontainer").remove(), wave.report.things.updateTopbar()
    } else wave.tabsengine.showWaveIcons(t)
}, wave.tabsengine.showWaveIcons = function(e) {
    var t = "extension" == waveconfig.platform ? document.body : document.getElementById("report").contentWindow.document;
    0 == e ? (t.querySelectorAll(".wave5icon, .wave5text").forEach((function(e) {
        e.setAttribute("hidden", "hidden")
    })), document.querySelectorAll("#wave5topbar .wave5icon, #wave5topbar .wave5text").forEach((function(e) {
        e.setAttribute("hidden", "hidden")
    })), wave.report.things.updateTopbar(), wave.tabsengine.WaveIconsVisible = !1, wave.tabsengine.showNavOrder("show", !1)) : (t.querySelectorAll(".wave5icon, .wave5text").forEach((function(e) {
        e.removeAttribute("hidden")
    })), document.querySelectorAll("#wave5topbar .wave5icon, #wave5topbar .wave5text").forEach((function(e) {
        e.removeAttribute("hidden")
    })), wave.report.things.updateTopbar(), wave.tabsengine.WaveIconsVisible = !0, 1 == wave.tabsengine.navOrderVisible && wave.tabsengine.showNavOrder("show", !0))
}, wave.tabsengine.scrollToNavIcon = function(e) {
    if ("extension" == waveconfig.platform) {
        var t = document.getElementById("waveorder" + e);
        window.scrollTo(0, t.getBoundingClientRect().top + document.documentElement.scrollTop - 30)
    } else {
        t = document.getElementById("report").contentWindow.document.getElementById("waveorder" + e);
        document.getElementById("report").contentWindow.document.documentElement.scrollTo(0, t.getBoundingClientRect().top + document.getElementById("report").contentWindow.document.documentElement.scrollTop - 30);
        var a = document.createElement("style");
        a.textContent = "@keyframes blink { 50% { opacity:0 ; } }", document.getElementById("report").contentWindow.document.body.appendChild(a)
    }
    t.style.animationPlayState = "running", t.style.animation = "blink .25s", t.style.animationIterationCount = "3", t.onanimationend = function(e) {
        e.target.style.animationPlayState = "paused", e.target.style.animation = "", "extension" != waveconfig.platform && a.remove()
    }
}, wave.tabsengine.buildNavItems = function(e) {
    var t = e.querySelectorAll('a[href]:not([href=""],#wave5codetogglelink),input:not([type=hidden],[type=HIDDEN],[disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), area, [contenteditable], [tabindex]:not([class=wave5icon],[disabled]), summary');
    t = Array.from(t).sort((function(e, t) {
        return (e = null == e.getAttribute("tabindex") || "0" == e.getAttribute("tabindex") ? 1e6 : e.getAttribute("tabindex")) - (t = null == t.getAttribute("tabindex") || "0" == t.getAttribute("tabindex") ? 1e6 : t.getAttribute("tabindex"))
    }));
    var a = {
            position: "absolute",
            color: "#fff",
    ...
    n.setAttribute("id", "waveordercontainer"), n.setAttribute("aria-hidden", "true"), Object.assign(n.style, {
        position: "absolute",
        top: 0,
        left: 0
    ...
    var i = [],
        ...
        r = -1,
        l = 1,
        c = "absolute" == getComputedStyle(e.body).getPropertyValue("position") ? 380 : 0;
    return t.forEach((function(e) {
        if (e.tabIndex >= 0 && !0 !== e.hidden && "none" !== getComputedStyle(e).getPropertyValue("display") && "hidden" !== getComputedStyle(e).getPropertyValue("visibility") && (0 != e.offsetWidth || 0 != e.offsetHeight)) {
            var t = "extension" == waveconfig.platform ? window.scrollX + e.getBoundingClientRect().left - c : document.getElementById("report").contentWindow.scrollX + e.getBoundingClientRect().left - c,
                d = "extension" == waveconfig.platform ? window.scrollY + e.getBoundingClientRect().top : document.getElementById("report").contentWindow.scrollY + e.getBoundingClientRect().top,
                u = document.createElement("div");
            u.classList.add("waveorder"), u.setAttribute("id", "waveorder" + l), l++, Object.assign(u.style, a), u.textContent = s, u.style.left = t + "px", u.style.top = d + "px", n.appendChild(u);
            var g = wave.tabsengine.getAccName(e);
            i.push(g.replace(/^./, (e => e.toUpperCase()))), o == d && r == t && (t = t + u.offsetWidth + 1, u.style.left = t + "px"), o = d, r = t, s++
        }
    })), i
    ...
    return n ? '"' !== n.substr(0, 1) ? "" : i ? n.slice(1, -1) : " " + n.slice(1, -1) + " " : ""
    ...
    if (!t && e.getAttribute("aria-labelledby")) {
        var o = e.getAttribute("aria-labelledby").split(/\s+/).map((function(e) {
            var t = document.getElementById(e);
            return t ? wave.tabsengine.getAccName(t, !0, a, !0) : ""
        }));
        s = o.join(" ")
    }
    if (!s.trim() && e.getAttribute("aria-label") && (s = e.getAttribute("aria-label")), !s.trim() && t && "LABEL" == e.tagName && ("true" != e.getAttribute("aria-hidden") && 1 != e.hidden && "none" != getComputedStyle(e).getPropertyValue("display") && "none" != getComputedStyle(e.parentNode).getPropertyValue("display") || (s = "*NO ACCESSIBLE NAME*")), !s.trim() && t && "OPTION" == e.tagName && (s = e.textContent), !s.trim() && !t && e.labels)
        ...
        s = r ? 0 == e.options[e.selectedIndex].text.length ? "*NO ACCESSIBLE NAME*" : e.options[e.selectedIndex].text : e.title || e.value || ""
    } else s = "BUTTON" === e.tagName || "submit" != e.type && "reset" != e.type ? "checkbox" == e.type || "radio" == e.type ? e.title || "" : e.placeholder || e.alt || e.value || e.title || "" : null != e.getAttribute("value") ? e.value : e.type || e.title || "";
    if (s.trim() || "ABBR" != e.tagName && "ACRONYM" != e.tagName || !e.title || (s = e.title), !s.trim()) {
        var l = {
            FIGURE: "figcaption",
            TABLE: "caption",
            FIELDSET: "legend"
    ...
    if (s.trim() || e.querySelectorAll("img:not([class=wave5icon], svg, [role=button][aria-label], [role=link][aria-label], [role=img][aria-label]").forEach((function(e, t) {
            if ("true" !== e.getAttribute("aria-hidden") && !0 !== e.hidden && "none" !== getComputedStyle(e).getPropertyValue("display")) {
                var n = e.getAttribute("alt");
                ...
                if (r && -1 == r.search(/^\s+$/i)) {
                    var l = e.getAttribute("aria-labelledby").split(/\s+/).map((function(e) {
                        var t = document.getElementById(e);
                        return t ? wave.tabsengine.getAccName(t, !0, a, !0) : ""
                    }));
                    s = l.join(" ")
        ...
        var u = e.cloneNode(!0);
        u.querySelectorAll("style, script, noscript, .wave5icon, .wave5text").forEach((function(e) {
            e.remove()
        })), s = (s = (s = (s = u.textContent).trim().replace(/\*aria-.*?\*/, "")).trim().replace(/\*role=.*?\*/, "")).trim().replace(/\*tabindex=.*?\*/, "")
    }
    "" == s && (s = "*NO ACCESSIBLE NAME*");
    var g = wave.tabsengine.getPseudoContent(e, ":before"),
        m = wave.tabsengine.getPseudoContent(e, ":after");
    if (t || i) return g + s + m;
    var p = e.getAttribute("role") ? e.getAttribute("role") : e.tagName.toLowerCase();
    return (p = "input" == (p = "a" == p ? "Link" : p) ? e.type + " " + p : p) + ": " + g + s + m
};
data = wave.results;
//console.log(data);
//console.log(JSON.stringify(wave.results));
//console.log(wave.results.statistics+"\\");
//console.log(wave.results.statistics[0]+"\\");