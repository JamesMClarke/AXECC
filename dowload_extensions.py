...
open('ExtensionInformation.csv', 'w').close()

folder = 'screen_reader_crx_files/'
...
for ext in extension_urls:  
    ...
    try:
        driver.get(ext)
        list_file = open('ExtensionInformation.csv', 'a', encoding='utf-8') #
        ...
        try:
            ...
            if os.path.isfile(folder+re.sub('[<>:"/\|?*,]',' ',extension_name)+".crx"):
                i = 1
                while os.path.isfile(folder+re.sub('[<>:"/\|?*,]',' ',extension_name)+'_'+str(i)+".crx"):
                    i += 1
                
                urllib.request.urlretrieve(url, folder+re.sub('[<>:"/\|?*,]',' ',extension_name)+'_'+str(i)+".crx")
                file_name  = re.sub('[<>:"/\|?*,]',' ',extension_name)+'_'+str(i)+".crx"
            else:
                urllib.request.urlretrieve(url, folder+re.sub('[<>:"/\|?*,]',' ',extension_name)+".crx")
...
# https://clients2.google.com/service/update2/crx?response=redirect&prodversion=57.0&x=id%3Dnckgahadagoaajjgafhacjanaoiihapd%26installsource%3Dondemand%26uc
